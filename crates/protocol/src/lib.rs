mod cdp;
pub use cdp::{Event, ProtocolError, Request, Response};
...
mod tests {
    ...
    #[test]
    fn removed_subsystem_switches_are_rejected() {
        for name in ["worker", "streaming_networking", "canvas"] {
            let mut config = serde_json::to_value(WorkerConfig::default()).unwrap();
            assert!(config.get(name).is_none());
            config[name] = serde_json::json!(false);
            assert!(serde_json::from_value::<WorkerConfig>(config).is_err());
        }
    }
}
