...
class ReadableStream {
    ...
    cancel(reason) {
        ...
        this.__closed = true;
        for (const pending of this.__pending.splice(0)) pending.resolve({ value: undefined, done: true });
        return Promise.resolve(this.__cancel?.(reason));
    }
    tee() {
        ...
        const branches = [0, 1].map(index => new ReadableStream({
            ...
            cancel(reason) {
                ...
                if (cancelled.every(Boolean)) return reader.cancel(reason);
            },
        }));
        (async () => {
            try {
                ...
                controllers.forEach((controller, index) => {
                    if (!cancelled[index]) controller.error(error);
                });
...
Object.defineProperty(Response.prototype, "body", {
    get() {
        if (this.__bodyStream === undefined) {
            const bytes = this.__bytes.slice();
            this.__bodyStream = new ReadableStream({ start(controller) { controller.enqueue(bytes); controller.close(); } });
        }
...
Object.defineProperties(Response.prototype, {
    clone: { value: function clone() {
        if (this.bodyUsed || this.__bodyStream?.locked) throw new TypeError("body has already been consumed");
        const result = cloneResponse.call(this);
        if (this.__bodyStream !== undefined) {
            const [original, copy] = this.__bodyStream.tee();
            this.__bodyStream = original;
            result.__bodyStream = copy;
        }
        return result;
