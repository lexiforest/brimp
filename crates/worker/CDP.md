...
| `Page.navigate` | Navigates through `runtime` and emits load events. |
...
`Brimp.configure` applies configuration before contexts/targets are created.
`Brimp.doctor` probes native dependencies; `Brimp.waitForNetworkIdle` waits for
a quiet interval. `Brimp.extract` accepts extraction options in `params` and
returns the extracted document for the attached page session.
