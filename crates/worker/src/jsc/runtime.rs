...
pub struct JsRuntime {
    context: NonNull<crate::jsc::ffi::OpaqueJSContext>,
...
impl JsRuntime {
    pub fn new() -> Result<Self, JsException> {
        ...
        let options_ready = OPTIONS_READY.get_or_init(|| {
            ...
            unsafe {
                #[cfg(target_os = "macos")]
                {
                    if !crate::jsc::ffi::allow_mach_exception_handlers() {
                        return false;
                    }
                }
                crate::jsc::ffi::JSCInitialize();
                crate::jsc::ffi::JSCSetOptions(c"useSharedArrayBuffer=true".as_ptr())
            }
        });
        if !options_ready {
        ...
        let context = NonNull::new(context.cast_mut()).ok_or_else(|| {
            JsException::from_message("JavaScriptCore failed to create a global context")
        })?;
        let runtime = Self {
            context,
    ...
    pub fn make_deferred_promise(&self) -> Result<DeferredPromise, JsException> {
        // SAFETY: this runtime owns a live context.
        unsafe { DeferredPromise::create(self.as_raw()) }
    }

    pub fn call_function(&self, function: &ProtectedJsObject) -> Result<JsValue<'_>, JsException> {
        self.call_function_with_arguments(function, &[])
    }
    ...
    pub fn call_function_with_string(
        ...
        let argument = self.string_value(value)?;
        self.call_function_with_arguments(function, &[argument.as_raw()])
    }
    ...
    pub fn call_function_with_string_and_object(
        ...
        let argument = self.string_value(value)?;
        self.call_function_with_arguments(function, &[argument.as_raw(), object.as_raw(self)])
    }

    fn string_value(&self, value: &str) -> Result<JsValue<'_>, JsException> {
        let value = JsString::new(value)?;
        // SAFETY: the string and context are live; root the new value for subsequent calls.
        let raw = unsafe { JSValueMakeString(self.as_raw(), value.as_raw()) };
        Ok(unsafe { JsValue::from_raw(self, raw) })
    }
    ...
    fn call_function_with_arguments(
        ...
        let mut exception = ptr::null();
        // SAFETY: callers supply rooted arguments in this context, and handle validates the function.
        let result = unsafe {
        ...
        // SAFETY: the result belongs to this live runtime.
        Ok(unsafe { JsValue::from_raw(self, result) })
    }

    pub fn make_object_with_prototype(
        &self,
        prototype: crate::jsc::JsObjectIdentity,
    ) -> ProtectedJsObject {
...
fn set_property(
    ...
    object: crate::jsc::ffi::JSObjectRef,
    name: &JsString,
    value: JSValueRef,
) -> Result<(), JsException> {
    let mut exception = ptr::null();
...
unsafe extern "C" fn console_log(
    context: JSContextRef,
    _function: crate::jsc::ffi::JSObjectRef,
    _this_object: crate::jsc::ffi::JSObjectRef,
...
unsafe extern "C" fn native_dispatch(
    context: JSContextRef,
    function: crate::jsc::ffi::JSObjectRef,
    this_object: crate::jsc::ffi::JSObjectRef,
    argument_count: usize,
    arguments: *const JSValueRef,
    ...
    // Keep returned object roots alive until conversion and handoff to JSC finish.
    let converted = match &result {
        Ok(value) => value.to_raw(context),
        Err(error) => Err(error.clone()),
    };
    match converted {
        Ok(value) => value,
        Err(error) => {
            let Ok(message) = JsString::new(&error.to_string()) else {
                // SAFETY: avoid unwinding through the C callback if string allocation fails.
                let value = unsafe { JSValueMakeUndefined(context) };
                if !exception.is_null() {
            ...
            let value = unsafe { crate::jsc::ffi::JSValueMakeString(context, message.as_raw()) };
            if !exception.is_null() {
...
pub(crate) fn exception_from_raw(context: JSContextRef, exception: JSValueRef) -> JsException {
    match value_to_string_without_exception(context, exception) {
        Some(message) => JsException::from_message(message),
        None => JsException::from_message("JavaScript exception could not be converted to text"),
    }
}
...
fn value_to_string_without_exception(context: JSContextRef, value: JSValueRef) -> Option<String> {
    ...
    let raw = unsafe { crate::jsc::ffi::JSValueToStringCopy(context, value, ptr::null_mut()) };
