...
use crate::runtime::{
    AutomationError, Browser, BrowserContext, CancellationToken, PageHandle, PageOptions,
    RemoteArgument, StoredCookie, TouchPoint,
};
use base64::Engine;
...
pub(crate) struct ConnectionState {
    ...
    browser_contexts: HashMap<String, BrowserContext>,
    events: EventQueue,
    next_page_id: u64,
    next_session_id: u64,
    next_loader_id: u64,
...
impl ConnectionState {
    ...
    pub(crate) async fn dispatch(&mut self, request: &Request) -> Response {
        ...
        let result = match request.method.as_str() {
            ...
            "Brimp.extract" => self.worker_extract(request).await,
    ...
    async fn query_selector(&self, request: &Request) -> Result<Value, DispatchError> {
        ...
        let source = render_script(
            include_str!("../js/cdp/query_selector.js"),
    ...
    async fn query_selector_all(&self, request: &Request) -> Result<Value, DispatchError> {
        ...
        let node_id = node_id.to_string();
        let source = render_script(
            include_str!("../js/cdp/query_selector_all.js"),
            &[("__NODE_ID__", &node_id), ("__SELECTOR__", &selector)],
        );
        let page = self.page_for_session(session)?.clone();
    ...
    async fn get_attributes(&self, request: &Request) -> Result<Value, DispatchError> {
        ...
        let node_id = node_id.to_string();
        let source = render_script(
            include_str!("../js/cdp/get_attributes.js"),
            &[("__NODE_ID__", &node_id)],
        );
        let page = self.page_for_session(session)?.clone();
    ...
    async fn get_content_quads(&self, request: &Request) -> Result<Value, DispatchError> {
        ...
        let source = render_script(
            include_str!("../js/cdp/get_content_quads.js"),
            &[("__NODE__", &node)],
        );
        let page = self.page_for_session(session)?.clone();
    ...
    async fn get_box_model(&self, request: &Request) -> Result<Value, DispatchError> {
        ...
        let node = remote_node_expression(&request.params)?;
        let source = render_script(
            include_str!("../js/cdp/get_box_model.js"),
            &[("__NODE__", &node)],
        );
        let page = self.page_for_session(session)?.clone();
        tokio::task::spawn_blocking(move || page.evaluate(source))
    ...
    async fn invoke_node_method(
        ...
        let source = render_script(
            include_str!("../js/cdp/invoke_node_method.js"),
    ...
    fn context_owned(&self, context_id: &str) -> Result<BrowserContext, DispatchError> {
    ...
    fn context_for_page_request(&self, request: &Request) -> Result<BrowserContext, DispatchError> {
        let target = self.page_target_for_session(self.session(request)?)?;
        self.context_owned(&target.browser_context_id)
    }

    fn storage_context(&self, request: &Request) -> Result<BrowserContext, DispatchError> {
    ...
    fn page_for_session(&self, session: &str) -> Result<&PageHandle, DispatchError> {
...
pub(crate) struct NavigationJob {
    ...
    task: tokio::task::JoinHandle<
        Result<(crate::runtime::NavigationResponse, String), AutomationError>,
    >,
}
pub(crate) struct NavigationCompletion {
    session: String,
    event_request_headers: Vec<(String, String)>,
    result: Result<(crate::runtime::NavigationResponse, String), DispatchError>,
}
impl NavigationJob {
...
fn set_cookie_params(context: BrowserContext, params: &Value) -> Result<(), DispatchError> {
    let cookies = params
        .get("cookies")
        .and_then(Value::as_array)
...
fn set_cookie_param(context: BrowserContext, cookie: &Value) -> Result<(), DispatchError> {
