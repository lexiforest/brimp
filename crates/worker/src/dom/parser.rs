...
use crate::dom::{BrowserDocument, NodeId};
...
/// Populates the root even when XML validation fails, then returns the validation result.
pub fn parse_xml_at_root(
    document: Rc<RefCell<BrowserDocument>>,
    xml: &str,
    root: NodeId,
) -> Result<(), roxmltree::Error> {
    let validation = roxmltree::Document::parse(xml).map(|_| ());
