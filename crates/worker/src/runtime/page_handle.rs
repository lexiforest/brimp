...
use crate::network::{HeaderList, ResourceRequest};

use serde::Serialize;
use serde_json::Value;

use crate::runtime::{
    ExtractedDocument, ExtractionOptions, NavigationError, NavigationResponse, Page,
    ScreenshotOptions, Viewport, browser::PageSetup,
};
...
pub use super::CancellationToken;
...
pub struct PageHandle {
    pub(super) control: Arc<PageControl>,
}
impl PageHandle {
    pub(super) fn launch(setup: PageSetup) -> Result<Self, AutomationError> {
        let (commands, receiver) = mpsc::channel();
        let (ready_sender, ready_receiver) = mpsc::sync_channel(1);
        let worker = std::thread::Builder::new()
            .name("brimp-page-owner".into())
            .spawn(move || run_page(setup, receiver, ready_sender))
...
fn run_page(
    ...
    let mut page = match setup.create() {
        Ok(page) => page,
        Err(error) => {
...
fn evaluate(page: &Page, expression: &str) -> Result<Value, AutomationError> {
    ...
    let source = render_script(
        include_str!("../../js/runtime/page_handle/evaluate_json.js"),
...
fn evaluate_remote(
    ...
    let body = render_script(
        include_str!("../../js/runtime/page_handle/evaluate_expression.js"),
...
fn call_function_remote(
    ...
    let arguments = serde_json::to_string(&arguments)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let body = render_script(
        include_str!("../../js/runtime/page_handle/call_function.js"),
        &[
...
fn evaluate_remote_maybe_await(
    ...
    let setup = render_script(
        include_str!("../../js/runtime/page_handle/await_promise.js"),
        &[("__BODY__", body)],
    );
    page.eval(&setup)
        .map_err(|error| AutomationError::JavaScript(error.to_string()))?;
    ...
    loop {
        let settled = page
            .eval(include_str!(
                "../../js/runtime/page_handle/promise_settled.js"
            ))
            .map_err(|error| AutomationError::JavaScript(error.to_string()))?
            .to_string()
            .map_err(|error| AutomationError::JavaScript(error.to_string()))?
    ...
    evaluate_remote_source(
        page,
        include_str!("../../js/runtime/page_handle/promise_result.js"),
        return_by_value,
        object_group,
    )
}
...
fn evaluate_remote_source(
    ...
    let object_group = serde_json::to_string(&object_group)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let source = render_script(
        include_str!("../../js/runtime/page_handle/evaluate_remote.js"),
        &[
            ("__BODY__", body),
...
fn remote_object_properties(
    ...
    let object_id = serde_json::to_string(object_id)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let source = render_script(
        include_str!("../../js/runtime/page_handle/remote_object_properties.js"),
        &[
...
fn describe_remote_node(page: &Page, object_id: &str) -> Result<Value, AutomationError> {
    let object_id = serde_json::to_string(object_id)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let source = render_script(
        include_str!("../../js/runtime/page_handle/describe_remote_node.js"),
        &[("__OBJECT_ID__", &object_id)],
    );
    let json = page
        .eval(&source)
...
fn resolve_remote_node(
    ...
    let body = render_script(
        include_str!("../../js/runtime/page_handle/resolve_remote_node.js"),
...
fn release_remote_object(page: &Page, object_id: &str) -> Result<bool, AutomationError> {
    let object_id = serde_json::to_string(object_id)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let source = render_script(
        include_str!("../../js/runtime/page_handle/release_remote_object.js"),
        &[("__OBJECT_ID__", &object_id)],
    );
    let value = page
        .eval(&source)
...
fn release_remote_object_group(page: &Page, object_group: &str) -> Result<usize, AutomationError> {
    let object_group = serde_json::to_string(object_group)
        .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
    let source = render_script(
        include_str!("../../js/runtime/page_handle/release_remote_object_group.js"),
        &[("__OBJECT_GROUP__", &object_group)],
    );
    let value = page
        .eval(&source)
