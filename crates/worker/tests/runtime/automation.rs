...
use brimp_lite_worker::runtime::{
    AutomationError, Browser, PageOptions, RemoteArgument, TouchPoint,
};
...
fn shared_automation_workflow_navigates_evaluates_screenshots_and_closes() {
    let browser = Browser::with_resource_loader(Arc::new(WorkflowLoader));
...
fn navigation_timeout_is_deterministic_and_close_remains_safe() {
    let browser = Browser::with_resource_loader(Arc::new(HangingLoader));
    let page = browser.new_page(PageOptions::default()).unwrap();
    assert!(matches!(
...
fn navigation_timeout_interrupts_non_yielding_javascript() {
    let browser = Browser::with_resource_loader(Arc::new(LoopingScriptLoader));
    let page = browser.new_page(PageOptions::default()).unwrap();
...
fn navigation_returns_raw_and_rendered_responses_without_raising_for_http_status() {
    let browser = Browser::with_resource_loader(Arc::new(ResponseLoader));
    let page = browser.new_page(PageOptions::default()).unwrap();
...
fn concurrent_pages_pin_distinct_proxies_and_share_context_cookies() {
    ...
    let browser = Browser::with_persona_and_network_config(
        brimp_lite_worker::persona::PersonaConfig::default(),
        brimp_lite_worker::network::CurlConfig::default(),
    )
    .unwrap();
...
fn trusted_input_state_machine_dispatches_human_event_sequences_and_defaults() {
    let browser = Browser::with_resource_loader(Arc::new(InputLoader));
    let page = browser.new_page(PageOptions::default()).unwrap();
...
#[test]
fn local_and_managed_pages_share_setup_and_have_explicit_lifetimes() {
    let browser = Browser::with_resource_loader(Arc::new(WorkflowLoader));
    let options = PageOptions::builder().viewport(480, 320).build();
    let mut local = browser.new_local_page(options.clone()).unwrap();
    let managed = browser.new_page(options.clone()).unwrap();
    assert_eq!(local.viewport().width, managed.viewport().unwrap().width);
    browser
        .default_context()
        .set_cookie("https://example.test/", "shared", "yes")
        .unwrap();
    let runtime = tokio::runtime::Builder::new_current_thread()
        ...
        .unwrap();
    runtime
        .block_on(local.goto("https://example.test/"))
        .unwrap();
    managed
    ...
    assert!(matches!(
        browser.new_page(options.clone()),
        Err(AutomationError::Closed)
    ));
    assert!(matches!(
        browser.new_local_page(options),
        Err(AutomationError::Closed)
    ));
    assert_eq!(local.eval("6 * 7").unwrap().to_number().unwrap(), 42.0);
}

#[test]
fn default_browser_preserves_local_and_managed_page_options() {
    let browser = Browser::new().unwrap();
    let options = PageOptions::builder().viewport(480, 320).build();
    let local = browser.new_local_page(options.clone()).unwrap();
    let managed = browser.new_page(options).unwrap();
    assert_eq!(local.viewport().width, 480.0);
    assert_eq!(managed.viewport().unwrap().width, 480.0);
}
