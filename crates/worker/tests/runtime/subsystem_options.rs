...
use brimp_lite_worker::runtime::{Browser, PageOptions, PersistentStorageOptions};
use http::StatusCode;
...
#[test]
fn browser_subsystems_are_available_by_default() {
    ...
    let page = browser.new_local_page(options).unwrap();
    let observed = page
        .eval(
            r#"JSON.stringify({
    ...
    assert_eq!(
        observed,
        r#"{"worker":"function","sharedWorker":"function","serviceWorker":true,"webSocket":"function","eventSource":"function","webTransport":"undefined","indexedDB":"undefined","caches":"undefined","storage":false,"canvas2d":"function","imageBitmap":"function","path2d":"function","webgpu":false,"audioContext":"undefined","mediaStream":"undefined"}"#,
    );
}

#[test]
fn persistent_storage_requires_explicit_options() {
...
fn subsystem_event_trust_capability_is_not_exposed_to_page_scripts() {
    let browser = Browser::with_resource_loader(Arc::new(UnusedLoader));
    let page = browser.new_local_page(PageOptions::default()).unwrap();
    assert_eq!(
        page.eval(
...
fn persistent_storage_apis_store_origin_partitioned_data() {
    ...
    let mut first = browser.new_local_page(options.clone()).unwrap();
    runtime
    ...
    let mut second = browser.new_local_page(options.clone()).unwrap();
    runtime
    ...
    let mut isolated = browser.new_local_page(options).unwrap();
    runtime
...
fn enabled_worker_uses_an_isolated_runtime_and_posts_messages() {
    ...
    let mut page = browser.new_local_page(PageOptions::default()).unwrap();
    let runtime = tokio::runtime::Builder::new_current_thread()
        .build()
        .unwrap();
...
fn named_shared_and_service_worker_realms_survive_page_lifetimes() {
    ...
    let mut second = browser.new_local_page(options).unwrap();
    runtime
...
fn enabled_fetch_exposes_body_chunks_before_completion() {
    ...
    assert!(!page.run_until_idle_for(Duration::from_millis(10)).unwrap());
