...
#[test]
fn default_response_streams_preserve_bytes_and_finish() {
    let browser = Browser::with_resource_loader(Arc::new(FetchLoader::default()));
    let mut page = browser.new_local_page(PageOptions::default()).unwrap();
    page.eval(r#"
        globalThis.bodyResult = "waiting";
        (async () => {
            const response = new Response(new Uint8Array([0, 255, 65]));
            const body = response.body;
            const copy = response.clone();
            const bytes = await response.arrayBuffer();
            const copied = await copy.arrayBuffer();
            const text = await new Response("ready").text();
            const empty = await new Response().text();
            bodyResult = JSON.stringify([Array.from(new Uint8Array(bytes)), Array.from(new Uint8Array(copied)), text, empty]);
        })().catch(error => bodyResult = String(error));
    "#).unwrap();
    assert!(page.run_until_idle_for(Duration::from_secs(1)).unwrap());
    assert_eq!(
        page.eval("bodyResult").unwrap().to_string().unwrap(),
        r#"[[0,255,65],[0,255,65],"ready",""]"#
    );
}

#[test]
fn fetched_response_clones_consume_the_complete_stream() {
    let browser = Browser::with_resource_loader(Arc::new(FetchLoader::default()));
    let mut page = browser.new_local_page(PageOptions::default()).unwrap();
    page.eval(
        r#"
        globalThis.cloneResult = "waiting";
        fetch("https://example.test/api").then(async response => {
            const copy = response.clone();
            const [original, cloned] = await Promise.all([response.json(), copy.json()]);
            cloneResult = JSON.stringify([original.answer, cloned.answer]);
        }).catch(error => cloneResult = String(error));
    "#,
    )
    .unwrap();
    assert!(page.run_until_idle_for(Duration::from_secs(1)).unwrap());
    assert_eq!(
        page.eval("cloneResult").unwrap().to_string().unwrap(),
        "[42,42]"
    );
}
