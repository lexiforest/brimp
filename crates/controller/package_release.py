...
def copy_licenses(root: Path, package: Path, jsc_library_dir: Path) -> None:
    ...
    shutil.copy2(
        root / "crates/worker/licenses/curl-impersonate-LICENSE",
        licenses,
    )
    defuddle = root / "assets/defuddle/0.19.3"
    shutil.copytree(defuddle / "licenses", licenses / "defuddle")
    shutil.copy2(defuddle / "NOTICE.md", licenses / "defuddle-NOTICE.md")
    fonts = root / "crates/worker/assets/fonts"
    ...
    shutil.copy2(root / "crates/controller/README.md", package / "README.md")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="brimp-controller-release-") as temporary:
