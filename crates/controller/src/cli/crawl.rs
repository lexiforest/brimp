...
use super::common::NavigationOptions;
use super::{
...
impl CrawlOptions {
    fn parse(arguments: &[String]) -> Result<Option<Self>, AutomationError> {
        ...
        let max_pages = parser
            ...
            .map_err(argument_error)?
            .unwrap_or(1_000);
        if concurrency == 0 || max_pages == 0 {
            return Err(AutomationError::InvalidInput(
                "--concurrency and --max-pages must be positive".into(),
            ));
        }
        let format = parser
...
pub(super) fn run(arguments: &[String]) -> Result<(), AutomationError> {
    ...
    let browser = Arc::new(options.navigation.worker.open(
...
fn run_tasks(
    ...
    let page_count = options.concurrency.min(tasks.len());
    let (sender, receiver) = mpsc::sync_channel(page_count);
