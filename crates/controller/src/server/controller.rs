...
use crate::transport::{SocketPair, Transport};
...
use brimp_protocol::MAX_FRAME_SIZE as MAX_IPC_MESSAGE;
    ...
    fn parse(arguments: impl Iterator<Item = String>) -> Result<Self, String> {
        ...
        while let Some(argument) = arguments.next() {
            ...
            match name {
                ...
                "--cdp" if inline.is_none() => config.cdp = true,
                "--worker-args" => {
                    if arguments_file.replace(value()?).is_some() {
                        return Err("--worker-args may be specified only once".into());
                    }
                }
        ...
        if let Some(path) = arguments_file {
            let mut arguments =
                crate::browser::WorkerOptions::read_arguments(std::path::Path::new(&path))
                    .map_err(|error| error.to_string())?;
            arguments.append(&mut config.worker_args);
            config.worker_args = arguments;
        }
        Ok(config)
    }
}
...
impl Worker {
    fn spawn(
        ...
        let transport: Arc<dyn Transport> =
            Arc::new(SocketPair::from_stream(stream).map_err(|error| error.to_string())?);
...
fn read_worker(
    ...
    loop {
        let payload = match reader.receive() {
