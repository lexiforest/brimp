...
fn parser_rejects_conflicts_duplicates_and_ambiguous_stdout_before_launch() {
    for arguments in [
        ...
        vec!["fetch", "http://unused.test/", "--output", "-"],
        vec![
            "fetch",
            "http://unused.test/",
            "--timeout",
...
#[test]
fn fetch_evaluation_keeps_structured_data_on_stdout() {
...
#[test]
fn fetch_inserts_cookie_option_into_the_browser_jar() {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
...
#[test]
fn fetch_has_browser_subsystems_and_configures_persistent_storage() {
    ...
    let output = binary()
        .args([
            ...
            "--eval",
            "localStorage.enabled = 'yes'; [typeof Worker, typeof WebSocket, typeof HTMLCanvasElement.prototype.getContext, typeof indexedDB, typeof navigator.storage]",
        ])
        .output()
        .unwrap();
    worker.join().unwrap();
    ...
    assert_eq!(
        ...
        serde_json::json!(["function", "function", "function", "object", "object"])
    );
    if storage.exists() {
...
#[test]
fn fetch_loads_the_versioned_persona_schema() {
    let unique = SystemTime::now()
        .duration_since(SystemTime::UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("brimp-persona-{unique}.json"));
    let mut persona: serde_json::Value =
        serde_json::from_str(include_str!("../../worker/src/persona/example.json")).unwrap();
    persona["base_headers"]["user_agent"] = serde_json::json!("CLI Persona/1");
    persona["navigator"]["platform"] = serde_json::json!("CLI-OS");
    persona["navigator"]["hardware_concurrency"] = serde_json::json!(6);
    persona["viewport"] = serde_json::json!({"width":640,"height":480,"device_scale_factor":2});
    std::fs::write(&path, persona.to_string()).unwrap();
...
#[test]
fn fetch_writes_screenshot_binary_and_protects_existing_output() {
    let unique = SystemTime::now()
        .duration_since(SystemTime::UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    ...
    let output = binary()
        .args(["fetch", &url, "--output", path.to_str().unwrap()])
        .output()
        .unwrap();
    worker.join().unwrap();
...
#[test]
fn fetch_preserves_binary_raw_output_through_the_configured_proxy() {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
    let (observed, request) = std::sync::mpsc::sync_channel(1);
    let worker = std::thread::spawn(move || {
...
#[test]
fn fetch_extracts_javascript_rendered_markdown_and_protects_existing_output() {
    let unique = SystemTime::now()
        .duration_since(SystemTime::UNIX_EPOCH)
        .unwrap()
        .as_nanos();
...
#[test]
fn fetch_serializes_prepared_html_and_structured_extraction_json() {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
    let worker = std::thread::spawn(move || {
        for _ in 0..2 {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let script = std::env::temp_dir().join(format!("brimp-fetch-{unique}.js"));
    fs::write(
        &script,
...
fn timeout_uses_stable_exit_category() {
    ...
    let output = binary()
        .args(["fetch", &url, "--eval", "1", "--timeout", "5s"])
        .output()
        .unwrap();
    drop(stop);
...
fn interrupt_cancels_navigation_and_releases_the_request() {
    ...
    let mut child = binary()
        .args(["fetch", &format!("http://{address}/"), "--eval", "1"])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .unwrap();
...
#[test]
fn removed_subsystem_flags_are_rejected_before_worker_launch() {
    for flag in [
        "--enable-worker",
