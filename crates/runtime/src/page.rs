...
use brimp_dom::{BrowserDocument, HtmlParserSession, NodeId, ParseProgress};
use brimp_jsc::{JsException, JsRuntime, JsValue, ProtectedJsObject};
use brimp_network::{HeaderList, NetworkError, ResourceLoader, ResourceRequest};
use brimp_web_apis::{
...
pub struct Page {
    ...
    persona: brimp_persona::ResolvedPersona,
    ...
    websockets: HashMap<u64, brimp_network::WebSocketHandle>,
    fetch_streams: RefCell<HashMap<u64, brimp_network::ResourceStreamHandle>>,
...
impl Page {
    ...
    async fn fetch_success(
        &self,
        url: &str,
    ) -> Result<brimp_network::ResourceResponse, NavigationError> {
    ...
    fn accept_success_response(
        &self,
        response: brimp_network::ResourceResponse,
    ) -> Result<brimp_network::ResourceResponse, NavigationError> {
                ...
                let result = loader.fetch_stream_callback(
                    ...
                    Box::new(move |event, control| {
                        ...
                        let chunk_length = match &event {
                            brimp_network::ResourceStreamEvent::Chunk(bytes) => bytes.len(),
                            _ => 0,
                        };
                        if chunk_length != 0 {
                            ...
                            if reserved.is_err() {
                                ...
                                if sender
                                    ...
                                    .is_err()
                                {
                                    return brimp_network::ResourceStreamDirective::Cancel;
                                }
                                return brimp_network::ResourceStreamDirective::Pause;
                            }
                        }
                        ...
                        brimp_network::ResourceStreamDirective::Continue
                    }),
                );
                match result {
                    Ok(handle) => {
    ...
    fn complete_service_worker_fetch(
        ...
        if streaming {
            ...
            self.complete_fetch(
                ...
                Ok((
                    String::new(),
                    brimp_network::ResourceResponse {
                        status,
                        headers,
    ...
    fn handle_fetch_stream_event(
        &self,
        id: u64,
        event: brimp_network::ResourceStreamEvent,
    ) -> Result<(), JsException> {
        match event {
            brimp_network::ResourceStreamEvent::Headers {
                status,
                mut headers,
                url,
            } => {
            ...
            brimp_network::ResourceStreamEvent::Chunk(bytes) => {
                self.bindings.deliver_fetch_stream_event(
                    &self.js,
                    id,
            ...
            brimp_network::ResourceStreamEvent::Complete => {
            ...
            brimp_network::ResourceStreamEvent::Error(error) => {
                self.fetch_streams.borrow_mut().remove(&id);
                if let Some(settlement) = self.fetches.borrow_mut().take_settlement(id) {
    ...
    fn finish_worker_start(
        ...
        name: String,
        scope: String,
        result: Result<brimp_network::ResourceResponse, String>,
    ) {
                    ...
                    let handle = self.network_scope.loader.open_websocket(
                        ...
                        Box::new(move |event| {
                            let event = match event {
                                brimp_network::WebSocketEvent::Open => serde_json::json!({"type": "open"}),
                                brimp_network::WebSocketEvent::Text(data) => serde_json::json!({"type": "message", "data": data}),
                                brimp_network::WebSocketEvent::Binary(data) => serde_json::json!({"type": "message", "data": data}),
                                brimp_network::WebSocketEvent::Close { code, reason } => serde_json::json!({"type": "close", "code": code, "reason": reason, "wasClean": code == 1000}),
                                brimp_network::WebSocketEvent::Error(message) => serde_json::json!({"type": "error", "message": message}),
                            }.to_string();
                            let _ = sender.post(move |page| {
    ...
    fn complete_fetch(
        &self,
        id: u64,
        result: Result<(String, brimp_network::ResourceResponse), String>,
    ) -> Result<(), JsException> {
...
struct LoadedScript {
    ...
    mode: ScriptMode,
    source: String,
    result: Result<brimp_network::ResourceResponse, NetworkError>,
}
...
impl Default for PageOptions {
    fn default() -> Self {
        let persona = brimp_persona::PersonaConfig::default().resolve();
        Self {
            viewport: Viewport {
...
impl PageOptions {
    ...
    pub fn persona(&self) -> &brimp_persona::ResolvedPersona {
        &self.persona
    }
    ...
    pub(crate) fn with_persona(mut self, persona: brimp_persona::ResolvedPersona) -> Self {
...
fn persona_request_headers(persona: &brimp_persona::ResolvedPersona) -> Vec<(String, String)> {
...
fn composite_canvas_rasters(
    ...
    target_width: u32,
    target_height: u32,
    layers: &[(brimp_web_apis::CanvasRaster, [f64; 4])],
    scale: f64,
) {
