...
use brimp_network::{HeaderList, ResourceLoader, ResourceRequest};
use brimp_web_apis::{CookieJar, StoredCookie};
use serde::Serialize;
...
impl AutomationBrowser {
    pub fn new() -> Result<Self, AutomationError> {
        let persona = brimp_persona::PersonaConfig::default();
    ...
    pub fn with_persona_and_resource_loader(
        persona: brimp_persona::PersonaConfig,
        loader: Arc<dyn ResourceLoader>,
    ...
    pub fn with_persona(persona: brimp_persona::PersonaConfig) -> Result<Self, AutomationError> {
        Self::with_persona_and_network_config(persona, brimp_network::CurlConfig::default())
    }
    pub fn with_persona_and_network_config(
        persona: brimp_persona::PersonaConfig,
        mut config: brimp_network::CurlConfig,
    ) -> Result<Self, AutomationError> {
        persona
            .validate()
            .map_err(|error| AutomationError::InvalidInput(error.to_string()))?;
        ...
        brimp_network::CurlResourceLoader::check_profile(&config)
            .map_err(|error| AutomationError::Transport(error.to_string()))?;
        let loader = Arc::new(
            brimp_network::CurlResourceLoader::new(config.clone())
                .map_err(|error| AutomationError::Transport(error.to_string()))?,
        );
        Ok(Self {
            loader,
    ...
    pub fn new_page_in_context_with_proxy(
        ...
        config.proxy = Some(
            brimp_network::Proxy::parse(proxy)
                .map_err(|error| AutomationError::InvalidInput(error.to_string()))?,
        );
        let loader = Arc::new(
            brimp_network::CurlResourceLoader::new(config)
                .map_err(|error| AutomationError::Transport(error.to_string()))?,
        );
    ...
    pub fn new_page_with_request_interceptor(
        &self,
        options: PageOptions,
        interceptor: Arc<dyn brimp_network::ResourceInterceptor>,
    ) -> Result<AutomationPage, AutomationError> {
        let loader = Arc::new(brimp_network::InterceptingResourceLoader::new(
