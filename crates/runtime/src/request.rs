...
use brimp_web_apis::BrowsingContext;
use std::sync::Arc;
...
pub(crate) struct RedirectHop {
    ...
    pub headers: brimp_network::HeaderList,
    pub url: String,
}
...
pub(crate) fn fetch_callback(
    ...
    request: ResourceRequest,
    callback: brimp_network::ResourceCallback,
) -> Result<(), NetworkError> {
...
struct RedirectTask {
    ...
    request: ResourceRequest,
    redirects: usize,
    callback: Arc<std::sync::Mutex<Option<brimp_network::ResourceCallback>>>,
}
impl RedirectTask {
...
fn finish_callback(
    callback: &std::sync::Mutex<Option<brimp_network::ResourceCallback>>,
...
mod tests {
    ...
    use brimp_network::{HeaderList, ResourceLoader};
